...
def search(query, maxResults=25, userId="me"):
    ...
    maxResults_ = min(maxResults, 500)

    response = SERVICE_GMAIL.users().threads().list(userId=userId, q=query, maxResults=maxResults_).execute()
    gmailThreads = response.get("threads", [])

    while "nextPageToken" in response and len(gmailThreads) < maxResults:
        response = (
            SERVICE_GMAIL.users()
            .threads()
            .list(userId=userId, q=query, maxResults=maxResults_, pageToken=response["nextPageToken"])
            .execute()
        )
